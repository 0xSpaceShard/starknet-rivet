...
export interface DeclareContractMessage extends Message<{
  sierra: string | CompiledContract;
  casm: CairoAssembly | undefined;
}> {}

export interface DeployContractMessage extends Message<{
    class_hash: string,
    call_data:  {
      [key: string]: any;
    };
...
export interface BlockIntervalMessage extends Message<{
  url: string;
  interval: number;
}> {}
...
export interface NewUrlToListtMessage extends Message<{
  item: ListOfDevnet;
}> {}

export interface SlectedAccountMessage extends Message<{
